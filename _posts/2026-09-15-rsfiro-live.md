...
Today there are already [live Uniswap pools on Ethereum](https://app.uniswap.org/explore/tokens/ethereum/0x2744eA5Ac9b11CB5E3CD63D3a88E858336aEddC2) and also [MewFinance on Ergo](https://dex.mewfinance.com/ergo/liquidity/d86f6508c6b665bf4ba0bd3b56f7090404665b0cef26e5202d91127ed538f47c) allowing you to immediately swap and/or provide liquidity today!
...
Rosen distributes responsibility across a federation of Guards (current Guard set is 10) rather than putting one operator in charge, while anyone with the requisite number of RSN can become a Watcher.
...
## Before You Bridge or Trade rsFIRO (ETH)

### Check the Official Token Address

Always verify the contract address before buying rsFIRO or adding it to your wallet. A token's name or symbol alone does not establish that it is genuine - fake tokens can copy both.
...
**This is the token's contract address on Ethereum, not a deposit address. Do not send funds directly to it.** For other supported chains, check the official token identifier for that network rather than assuming the Ethereum address applies.

### Keep ETH Available for Transaction Fees

You will need **ETH in the same wallet on Ethereum mainnet** to send or swap rsFIRO, provide liquidity, or initiate a bridge transfer back. These network fees are paid in ETH, not rsFIRO. When providing liquidity, keep some ETH aside for future transactions rather than committing your entire balance to the pool. You can do so by using a swap service such as [wizardswap.io](https://wizardswap.io) or [exolix.com](https://exolix.com) to swap some native FIRO to ETH to your ETH wallet address.
